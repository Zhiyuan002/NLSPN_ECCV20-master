...
from model.training.diffaug import DiffAugment
from model.training.adaaug import AdaAugment
...
class Diffusion(torch.nn.Module):
    def __init__(self,
        ...
        t_min=10, t_max=1000, noise_std=0.08,
    ...
    def forward(self, x_0, x_1, x_2):
        ...
        t = torch.from_numpy(np.random.choice(self.t_epl, size=batch_size, replace=True)).to(device)
        x_t0 = q_sample(x_0, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t,
                       noise_type=self.noise_type,
                       noise_std=self.noise_std)
        x_t1 = q_sample(x_1, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t,
                       noise_type=self.noise_type,
                       noise_std=self.noise_std)
        x_t2 = q_sample(x_2, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t,
                       noise_type=self.noise_type,
                       noise_std=self.noise_std)
        return x_t0 ,x_t1, x_t2, t.view(-1, 1)
#----------------------------------------------------------------------------
